<?php
    ...
    $json = file_get_contents("https://api.openweathermap.org/data/2.5/weather?q=Praha&APPID=347daf3f92c0d8652b70ee139b366eb1&units=metric");
    ...
    if (isset($obj->{'dt'})) {
        echo "dt: ".$obj->{'dt'}." (".date("Y-m-d H:i:s", $obj->{'dt'}).")"."<br />";
        $wDt = $obj->{'dt'};
    }
    ...
    if (isset($obj->{'sys'})) {
        ...
        if (isset($obj->{'sys'}->{'sunrise'})) {
            echo "sys->sunrise: ".$obj->{'sys'}->{'sunrise'}." (".date("Y-m-d H:i:s", $obj->{'sys'}->{'sunrise'}).")"."<br />";
            $wSysSunrise = $obj->{'sys'}->{'sunrise'};
        }
        if (isset($obj->{'sys'}->{'sunset'})) {
            echo "sys->sunset: ".$obj->{'sys'}->{'sunset'}." (".date("Y-m-d H:i:s", $obj->{'sys'}->{'sunset'}).")"."<br />";
