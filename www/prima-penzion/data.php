<?php
    ...
    $instanceDb = new PDO(
        "mysql:host=localhost;dbname=funkacerpenzion;charset=utf8",
        "funkacerpenzion",
        "1bdeZDB8JN",
    ...
    /*
    $instanceDb = new PDO(
        "mysql:host=localhost:8889;dbname=penzion;charset=utf8",
        "root",
        "root",
        array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION)
    );
    */
    ...
    class Kontakt {
        ...
        public function __construct($argId, $argJmeno, $argPrijmeni, $argEmail, $argVzkaz)
        {
            $this->id = $argId;
            $this->jmeno = $argJmeno;
        ...
        public function ulozDoDb() {
            if ($this->id == null) {
                //insert
                $prikaz = $GLOBALS["instanceDb"]->prepare("INSERT INTO kontakt SET id = ?, jmeno = ?, prijmeni = ?, email = ?, vzkaz = ?");
                $prikaz->execute([$this->id, $this->jmeno, $this->prijmeni, $this->email, $this->vzkaz]);
            } else {
                //update
            }
        }
